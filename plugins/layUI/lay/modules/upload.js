...
layui.define('layer' , function(exports){
  ...
  Class.prototype.file = function(){
    var that = this
    ,options = that.config
    ,elemFile = that.elemFile = (function () {
    	var next = $(options.elem).next()[0];
    	return next && next.type == "file" && next.nodeName.toLowerCase() == "input"? 
    	$(next).addClass(ELEM_FILE)
    	: $([
    ...
    switch(options.accept){
      case 'file': //一般文件
        if(exts && !RegExp('\\.('+ exts +')$', 'i').test(escape(value))){
          ...
          if(options.msg){
              ...
              // 扩展定义,如果根据支持的扩展名来自适应信息,将做出相应的处理。
              if(options.extsText){
                  that.msg("文件格式只能是" + exts.replace(/\|/g,",") + "其中的一种");
              }else{
      ...
      default: //图片文件
        layui.each(value, function(i, item){
          if(!RegExp('\\.('+ (exts || 'jpg|png|gif|bmp|jpeg|pdf$') +')', 'i').test(escape(item))){
            check = true;
          }
        });
        if(check){
  ...
  Class.prototype.events = function(){
    ...
    ,setChooseText = function(files, filename){
      ...
      files = elemFile[0].files;
      var value = files.length > 1 
        ? files.length + '个文件' 
      : ((files[0] || {}).name || (elemFile[0].value.match(/[^\/\\]+\..+/g)||[]) || '');
      if(that.isFile() || options.choose) return;
      elemFile.after('<span class="'+ ELEM_CHOOSE +'">'+ value +'</span>');
    };
