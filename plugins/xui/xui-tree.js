      ...
      var li = $(['<li '+ (item.spread ? 'data-spread="'+ item.spread +'"' : '') +'>'
        ...
        ,function(){
          ...
          + ('<cite>'+ (item.name||'未命名') +'</cite></a>')
              // + ( item.id == 1 ? '': '<input type="checkbox" value="'+item.id+'" name= "showLevelList"  class= '+( item.parentId == 1 ?  "showLevelParentList" : "showLevelList")+' lay-filter="showLevelList" lay-skin="primary" '+ (item.showLevelCheck == 0 ? "" : "checked") +' title="1"/>' )
              ;
        }()
