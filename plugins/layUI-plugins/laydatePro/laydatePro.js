 ...
 ;!function (factory) {
  ...
  var
    ...
    init = function (layThat, change) {
      ...
      // 俩月
      if (options.type === 'month' && options.subType === 'twoMonth') {

        lay.each(elemContent.find('.laydate-month-list').find('li'), function (index, monElem) {
          monElem = $(monElem);
          var monthTemp = parseInt(monElem.attr('lay-ym'));
          if (monthTemp < 6) {
            monElem.html(twoMonthObj[monthTemp]);
          } else {
            monElem.hide();
          }
        });
        if (!change && !elemTarget.val()) {
          ...
          lay.each(elemContent.find('.laydate-month-list').find('li.layui-this'), function (index, monElem) {
            ...
            // 获得选中的月份
            var month = parseInt(monElem.attr('lay-ym'));
            monElem.closest('.laydate-month-list').find('li[lay-ym="' + month + '"]').click();
      ...
      , initDate = function (dateTime, value, index) {
        ...
        lay.each(that.format, function (i, item) {
          ...
          if (/yyyy|y/.test(item)) { //年
            ...
            if('twoMonth' === options.subType) {
              dateTime.month = Math.floor((thisv - 1)/ 2);
            } else {
      ...
      if('twoMonth' === options.subType) {
        ...
        lay.each(format, function (i, item) {
          if (/yyyy|y/.test(item)) { //年
            ...
            if(isSecondmonth) {// 第二个月份+1
              format[i] = lay.digit( 2 * dateTime.month + 2 , item.length);
            } else {
              format[i] = lay.digit( 2 * dateTime.month + 1, item.length);
            }
            isSecondmonth = true;
          } else if (/dd|d/.test(item)) { //日
  ...
  laydate.render = function (config) {
    ...
    $.each($(config.elem), function (index, elem) {
      ...
      // 俩月
      if (configNew.type === 'twoMonth') {
        configNew.type = 'month';
        configNew.subType = 'twoMonth';
        configNew.format = configNew.format || 'yyyy年M~M月';
      }
